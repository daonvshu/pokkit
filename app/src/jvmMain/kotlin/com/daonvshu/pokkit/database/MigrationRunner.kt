            ...
            try {
                ...
                if (appliedVersions.isEmpty()) {
                    ...
                    SchemaMigrations.insert {
                        it[SchemaMigrations.version] = migrations.last().version
                        it[SchemaMigrations.description] = "init database version."
                    }
                    return@transaction
                }
                ...
                pendingMigrations.sortedBy { it.version }.forEach { migration ->
                    ...
                    SchemaMigrations.insert {
                        it[SchemaMigrations.version] = migration.version
                        it[SchemaMigrations.description] = migration.description + if (skipped) " (skipped)" else ""
                    }
                }
            } catch (e: Exception) {
                e.printStackTrace()
