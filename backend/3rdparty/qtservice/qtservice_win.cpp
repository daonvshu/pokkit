...
{
    if (!pOpenSCManager) {
        QLibrary lib(QLatin1String("advapi32"));
        ...
        if (hService) {
            ...
            if (pQueryServiceConfig(hService, (LPQUERY_SERVICE_CONFIG)data, 8 * 1024, &sizeNeeded)) {
                ...
                result = QString::fromUtf16(
                        reinterpret_cast<const char16_t *>((const ushort *) config->lpBinaryPathName));
            }
            pCloseServiceHandle(hService);
        }
        pCloseServiceHandle(hSCM);
        ...
        if (hService) {
            ...
            if (pQueryServiceConfig2(
                ...
                if (desc->lpDescription)
                    result = QString::fromUtf16(
                            reinterpret_cast<const char16_t *>((const ushort *) desc->lpDescription));
            }
            pCloseServiceHandle(hService);
        }
        pCloseServiceHandle(hSCM);
...
void WINAPI QtServiceSysPrivate::serviceMain(DWORD dwArgc, wchar_t** lpszArgv)
{
    ...
    for (DWORD i = 0; i < dwArgc; i++)
        instance->serviceArgs.append(QString::fromUtf16(
                reinterpret_cast<const char16_t *>((unsigned short *) lpszArgv[i])));
    ...
    void run()
        {
            ...
            if (!success) {
                if (GetLastError() == ERROR_FAILED_SERVICE_CONTROLLER_CONNECT) {
                    ...
                    QtServiceBase::instance()->logMessage(QLatin1String("The Service failed to start [%1]").arg(qt_error_string(GetLastError())), QtServiceBase::Error);
                }
...
QString QtServiceBasePrivate::filePath() const
{
    ...
    return QString::fromUtf16(reinterpret_cast<const char16_t *>((unsigned short *) path));
}
